...
// Get lessons
$lessons = ACM_Progress::get_instance()->get_course_chapters($course_id);
...
if ($related_only_mode && function_exists('acm_is_chapter_related_for_user')) {
    $lessons = array_values(array_filter($lessons, function($lesson) use ($user_id) {
        return acm_is_chapter_related_for_user($lesson->ID, $user_id);
    }));
}
...
if ($is_logged_in) {
    ...
    $is_course_complete = $acm_certificate->is_course_completed($user_id, $course_id);
    
    if ($is_course_complete) {
        $certificate = $acm_certificate->get_certificate($user_id, $course_id);
    }
}
...
<style>
    ...
    .course-start-card .acm-btn {
        font-size: 16px;
        font-weight: 600;
        padding: 14px 24px;
        gap: 10px;
    }
    .course-start-card .btn-arrow {
        font-size: 20px;
        line-height: 1;
    }
</style>
<div class="acm-course-page acm-filter-root">
        ...
        <div class="course-header-overlay">
            <div class="course-header-content">
                <nav class="course-breadcrumb" aria-label="Breadcrumb">
                    ...
                    <a href="<?php echo get_post_type_archive_link('acm_course'); ?>"><?php _e('Course', 'advanced-course-manager'); ?></a>
                    <span class="separator">/</span>
                ...
                <div class="course-meta">
                    <?php if ($total_hours): ?>
                        <div class="meta-item" style="align-items: inherit;">
                            ...
                            <span class="meta-text"><?php echo esc_html($total_hours); ?> <?php _e('hours', 'advanced-course-manager'); ?></span>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="meta-item" style="align-items: inherit;">
                        ...
                        <span class="meta-text"><?php echo esc_html($chapter_count); ?> <?php _e('Chapters', 'advanced-course-manager'); ?></span>
                    </div>
        ...
        <main class="acm-course-main" role="main">
            ...
            <?php if ($is_logged_in): ?>
                ...
                <?php if ($customization['completed']): ?>
                    <div class="course-actions">
                        <a href="<?php echo esc_url(acm_get_customization_quiz_link()); ?>" class="acm-btn acm-btn-secondary" aria-label="Customize course">
            ...
            <div class="course-customization-notice" role="alert">
                ...
                <div class="notice-content">
                    ...
                    <a href="<?php echo esc_url(acm_get_customization_quiz_link()); ?>" aria-label="View all course sections">
    ...
    <div class="courses-section">
            ...
            <div id="courses-grid-container">
                <?php if (!empty($lessons)): ?>
                    ...
                    <div class="courses-grid">
                        <?php 
                            ...
                            $cpt_number = get_post_meta($lesson->ID, '_acm_chapter_number', true);
                            ...
                            $lesson_count_chapter = 0;
                            $total_lesson_time_ch = 0;

                            $lesson_ids_chapter = get_posts(array(
                                'post_type'      => 'acm_lesson',
                                'posts_per_page' => -1,
                                'post_status'    => 'publish',
                                'fields'         => 'ids',
                            ...
                            $lesson_count_chapter = count($lesson_ids_chapter);
                            foreach ($lesson_ids_chapter as $lesson_id_ch) {
                                $raw_duration_ch = get_post_meta($lesson_id_ch, '_acm_lesson_duration', true);
                                if (empty($raw_duration_ch)) {
                                    continue;
                                }
                                if (is_string($raw_duration_ch)) {
                                    preg_match('/\d+/', $raw_duration_ch, $m_ch);
                                    $raw_duration_ch = $m_ch[0] ?? 0;
                                }
                                $duration_ch = (int) $raw_duration_ch;
                                if ($duration_ch > 0) {
                                    $total_lesson_time_ch += $duration_ch;
                                }
                            }
                            $total_hours_ch = $total_lesson_time_ch > 0 ? round($total_lesson_time_ch / 60, 1) : 0;
                            ...
                            $chapter_total_lessons     = count($chapter_lesson_ids);
                            $chapter_completed_lessons = 0;

                            if ($is_logged_in && $chapter_total_lessons > 0) {
                                foreach ($chapter_lesson_ids as $lesson_id) {
                                    $lp = ACM_Progress::get_instance()->get_lesson_progress($user_id, $lesson_id);
                            ...
                            $chapter_progress_pct = $chapter_total_lessons
                                ? round(($chapter_completed_lessons / $chapter_total_lessons) * 100)
                                : 0;

                            $chapter_hidden_class = function_exists('acm_get_hidden_class')
                                ? acm_get_hidden_class($lesson->ID, $user_id)
                                : '';
                            $chapter_link = get_permalink($lesson->ID);
                            if ($related_only_mode) {
                                $chapter_link = add_query_arg('acm_related_only', '1', $chapter_link);
                            }
                        ?>
                        
                            <div class="course-card-modern<?php echo esc_attr($chapter_hidden_class); ?>" data-acm-item-id="<?php echo esc_attr($lesson->ID); ?>" data-acm-filter-key="<?php echo esc_attr(function_exists('acm_get_filter_key') ? acm_get_filter_key($lesson->ID) : ''); ?>">
                                ...
                                <div class="course-info">
                                    <h3 class="course-title">
                                        <a href="<?php echo esc_url($chapter_link); ?>" aria-label="Go to lesson <?php echo esc_attr($lesson->post_title); ?>"><?php echo esc_html($lesson->post_title); ?></a>
                                    </h3>

                                    <?php
                                    $lesson_excerpt = get_post_field('post_excerpt', $lesson->ID);
                                    if (!empty($lesson_excerpt)):
                                    ?>
                                        <p class="course-excerpt">
                                            <?php echo wp_trim_words($lesson_excerpt, 20); ?>
                                        </p>
                                    <?php endif; ?>
                                    ...
                                    <div class="course-meta-grid">
                                        ...
                                        <?php if ($cpt_number): ?>
                                            <div class="meta-badge">
                                                <span>Chapter <?php echo esc_html($cpt_number); ?></span>
                                            </div>
                                        <?php endif; ?>

                                        <?php if ($lesson_count_chapter > 0){ ?>
                                            <div class="meta-badge">
                                                <span class="meta-icon">📚</span>
                                                <span><?php echo $lesson_count_chapter; ?> Lessons</span>
                                            </div>
                                        <?php } else { ?>
                                            <div class="meta-badge">
                                                <span class="meta-icon">📚</span>
                                        ...
                                        <?php if ($total_hours_ch): ?>
                                            <div class="meta-badge">
                                                <span class="meta-icon">⏱️</span>
                                                <span><?php echo $total_hours_ch; ?>h</span>
                                            </div>
                                        <?php else: ?>
                                            <div class="meta-badge">
                                                <span class="meta-icon">⏱️</span>
                                    ...
                                    <?php if ($chapter_progress_pct > 0): ?>
                                        <div class="course-progress-card">
                                            <div class="progress-header">
                                                <span class="progress-label"><?php _e('Your Progress', 'advanced-course-manager'); ?></span>
                                                <span class="progress-percent"><?php echo $chapter_progress_pct; ?>%</span>
                                            </div>
                                            <div class="progress-bar-modern">
                                                <div class="progress-fill-modern" style="width: <?php echo $chapter_progress_pct; ?>%"></div>
                                            </div>
                                        </div>
                                    <?php endif; ?>
                                    ...
                                    <div class="course-action">
                                        <?php if (!$has_access): ?>
                                            <a href="<?php echo esc_url($chapter_link); ?>" class="btn-modern btn-primary">
                                        ...
                                        <?php elseif ($chapter_progress_pct > 0): ?>
                                            <a href="<?php echo esc_url($chapter_link); ?>" class="btn-modern btn-primary">
                                                <span><?php _e('Continue Learning', 'advanced-course-manager'); ?></span>
                                                <svg class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: #555a5a;">
                                                    <path d="M5 12h14M12 5l7 7-7 7"/>
                                                </svg>
                                            </a>
                ...
                <?php else: ?>
                    ...
                    <div class="empty-state">
                        ...
                        <h3><?php _e('No Chapter Found', 'advanced-course-manager'); ?></h3>
                    </div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <aside class="acm-course-sidebar" role="complementary" aria-label="Course sidebar">
            <div class="sidebar-card course-start-card">
                <?php if ($is_logged_in): ?>
                    ...
                    <?php elseif ($progress['percentage'] >= 100): ?>
                        <h3><?php _e('Course Completed!', 'advanced-course-manager'); ?></h3>
                        <p><?php _e('Congratulations on completing this Course!', 'advanced-course-manager'); ?></p>
                        <?php if ($agreement_count > 0): ?>
            ...
            <?php if ($is_logged_in && $is_course_complete): ?>
                <div class="sidebar-card certificate-card">
                    ...
                    <?php if ($certificate): ?>
                        <p class="certificate-message">
                            <?php _e('Congratulations! You have completed this Course.', 'advanced-course-manager'); ?>
                        </p>
